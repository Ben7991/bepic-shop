<x-layouts.dashboard>
    ...
    <div
        ...
        <h4 class="font-bold text-[1.2em] mb-4">Edit membership package</h4>
        <form action="/dashboard/membership-packages/{{ $package->id }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label for="point" class="mb-1 inline-block">Points</label>
                <input type="number" name="point" id="point" value="{{ $package->point }}"
            ...
            <div class="mb-3">
                ...
                <input type="number" name="quantity" id="quantity" value="{{ $package->product_quantity }}"
                    class="flex items-center px-4 border rounded-lg gap-2 form-control w-full py-2 outline-none border-[var(--gray-300)] focus:border-[var(--sea-blue-100)]" />
                @error('quantity')
                    <small class="text-red-700">{{ $message }}</small>
                @enderror
